...
contract IBasicMultiToken is ERC20 {
    ...
    function allTokensDecimalsBalances()
        public
        view
        returns (ERC20[], uint8[], uint256[]);
...
contract IMultiToken is IBasicMultiToken {
    ...
    function allTokensDecimalsBalancesWeights()
        public
        view
        returns (
            ERC20[] _tokens,
    ...
    function buy(
        ...
        for (uint i = 0; i < _exchanges.length; i++) {
            bytes memory data = new bytes(
                _datasIndexes[i + 1] - _datasIndexes[i]
            );
            ...
            if (_throughToken != address(0) && i > 0) {
                _throughToken.approve(
                    _exchanges[i],
                    _throughToken.balanceOf(this)
                );
            }
            require(
