...
library Contract {
    ...
    bytes32 internal constant EXEC_PERMISSIONS =
        keccak256("script_exec_permissions");
    ...
    ) internal view conditions(validState, validState) {
        _check();
    }
    ...
    // is executed.
    function checks(
        function() pure _check
    ) internal pure conditions(validState, validState) {
        _check();
    }

    // Ensures execution completed successfully, and reverts the created storage buffer
    // back to the sender.
    function commit() internal pure conditions(validState, none) {
    ...
    function validState() private pure {
        if (freeMem() < 0x180) revert("Expected Contract.execute()");
    ...
    function currentAction() private pure returns (bytes4 action) {
        if (buffPtr() == bytes32(0)) return bytes4(0);
    ...
    function validStoreBuff() private pure {
        ...
        if (buffPtr() == bytes32(0)) startBuffer();
    ...
    bytes4 internal constant EMITS =
        bytes4(keccak256("Emit((bytes32[],bytes)[])"));
    ...
    function validStoreDest() private pure {
        ...
        if (expected() != NextFunction.STORE_DEST)
            revert(
                "Unexpected function order - expected storage destination to be pushed"
            );
    ...
    function validStoreVal() private pure {
        ...
        if (
            ...
            revert(
                "Unexpected function order - expected storage value to be pushed"
            );
    ...
    function validPayDest() private pure {
        ...
        if (expected() != NextFunction.PAY_DEST)
            revert(
                "Unexpected function order - expected payment destination to be pushed"
            );
    ...
    function validPayAmt() private pure {
        ...
        if (expected() != NextFunction.PAY_AMT)
            revert(
                "Unexpected function order - expected payment amount to be pushed"
            );
    ...
    function storing() internal pure conditions(validStoreBuff, isStoring) {
    ...
    function to(
        bytes32,
        bytes32 _val
    ) internal pure conditions(validStoreVal, validStoreDest) {
        assembly {
            // Get pointer to buffer length -
            let ptr := add(0x20, mload(0xc0))
    ...
    function to(bytes32 _field, bool _val) internal pure {
        to(_field, _val ? bytes32(1) : bytes32(0));
    }
    ...
    function by(
        ...
        if (expected() == NextFunction.VAL_INC) _amt = _amt.add(uint(_val));
        else if (expected() == NextFunction.VAL_DEC)
            _amt = uint(_val).sub(_amt);
        else revert("Expected VAL_INC or VAL_DEC");
    ...
    function byMaximum(
        ...
        if (expected() == NextFunction.VAL_DEC) {
            if (_amt >= uint(_val)) _amt = 0;
            else _amt = uint(_val).sub(_amt);
        } else {
    ...
    function emitting() internal pure conditions(validEmitBuff, isEmitting) {
    ...
    function log(
        bytes32 _data
    ) internal pure conditions(validEvent, validEvent) {
        assembly {
            // Get pointer to buffer length -
            let ptr := add(0x20, mload(0xc0))
    ...
    function paying() internal pure conditions(validPayBuff, isPaying) {
    ...
    function pay(
        uint _amount
    ) internal pure conditions(validPayAmt, validPayDest) returns (uint) {
        assembly {
            // Get pointer to buffer length -
            let ptr := add(0x20, mload(0xc0))
    ...
    function toAcc(
        uint,
        address _dest
    ) internal pure conditions(validPayDest, validPayAmt) {
        assembly {
            // Get pointer to buffer length -
            let ptr := add(0x20, mload(0xc0))
    ...
    function emitted() internal pure returns (uint num_emitted) {
        if (buffPtr() == bytes32(0)) return 0;

        // Load number emitted from buffer -
        assembly {
            num_emitted := mload(0x140)
        }
    }
...
library ManageTokens {
    ...
    bytes32 internal constant TRANSFER_AGENT_STATUS =
        keccak256("TransferAgentStatusUpdate(bytes32,address,bool)");
    // event CrowdsaleFinalized(bytes32 indexed exec_id)
    bytes32 internal constant FINAL_SEL =
        keccak256("CrowdsaleFinalized(bytes32)");
    // event TokenConfigured(bytes32 indexed exec_id, bytes32 indexed name, bytes32 indexed symbol, uint decimals)
    bytes32 private constant TOKEN_CONFIGURED =
        keccak256("TokenConfigured(bytes32,bytes32,bytes32,uint256)");

    // Returns the topics for a transfer agent status update event -
    function AGENT_STATUS(
        bytes32 _exec_id,
        address _agent
    ) private pure returns (bytes32[3] memory) {
    ...
    function FINALIZE(
        bytes32 _exec_id
    ) private pure returns (bytes32[2] memory) {
    ...
    function TOKEN_INIT(
        ...
        bytes32 _name,
        bytes32 _symbol
    ) private pure returns (bytes32[4] memory) {
    ...
    function initCrowdsaleToken(
        ...
        Contract.log(
            TOKEN_INIT(Contract.execID(), _name, _symbol),
            bytes32(_decimals)
        );
    }
    ...
    function setTransferAgentStatus(
        ...
        // Ensure valid input
        if (_agent == 0) revert("invalid transfer agent");
        ...
        Contract.log(
            AGENT_STATUS(Contract.execID(), _agent),
            _is_agent ? bytes32(1) : bytes32(0)
        );
    }
    ...
    function updateMultipleReservedTokens(
        ...
        uint num_destinations = uint(
            Contract.read(TokenManager.reservedDestinations())
        );
        ...
        for (uint i = 0; i < _destinations.length; i++) {
            ...
            if (to_add == 0) revert("invalid destination");
                ...
                Contract
                    .set(
                        bytes32(
                            32 *
                                num_destinations +
                                uint(TokenManager.reservedDestinations())
                        )
                    )
                    .to(to_add);
                // Store reservation index
                Contract.set(TokenManager.destIndex(to_add)).to(
                    num_destinations
                );
            }
            ...
            Contract.set(TokenManager.destPrecision(to_add)).to(
                _percent_decimals[i]
            );
        }
    ...
    function removeReservedTokens(address _destination) internal view {
        // Ensure valid input
        if (_destination == 0) revert("invalid destination");

        Contract.storing();

        // Get reservation list length
        uint reservation_len = uint(
            Contract.read(TokenManager.reservedDestinations())
        );
        // Get index of passed-in destination. If zero, sender is not in reserved list - revert
        uint to_remove = uint(
            Contract.read(TokenManager.destIndex(_destination))
        );
    ...
    function distributeReservedTokens(uint _num_destinations) internal view {
        // Ensure valid input
        if (_num_destinations == 0) revert("invalid number of destinations");
        ...
        uint total_supply = uint(
            Contract.read(TokenManager.tokenTotalSupply())
        );
        uint reserved_len = uint(
            Contract.read(TokenManager.reservedDestinations())
        );
        ...
        if (reserved_len == 0) revert("no remaining destinations");

        // If num_destinations is greater than the reserved destinations list length, set amt equal to the list length
        if (_num_destinations > reserved_len) _num_destinations = reserved_len;

        // Update reservedDestinations list length
        Contract.decrease(TokenManager.reservedDestinations()).by(
            _num_destinations
        );
        ...
        for (uint i = 0; i < _num_destinations; i++) {
            ...
            uint precision = uint(
                Contract.read(TokenManager.destPrecision(addr))
            ).add(2);
            ...
            to_add = to_add.add(
                uint(Contract.read(TokenManager.destTokens(addr)))
            );
...
library TokenManager {
    ...
    bytes32 internal constant TOKEN_TRANSFER_AGENTS =
        keccak256("token_transfer_agents");
    ...
    function saleFinalized() internal view {
        if (Contract.read(isFinished()) == 0) revert("sale must be finalized");
    }
    ...
    function onlyStores() internal pure {
        ...
        if (Contract.stored() == 0) revert("expected storage");
    }
  ...
  */
    function updateMultipleReservedTokens(
        ...
        Contract.checks(onlyAdminAndNotInit);
        // Execute function -
        ManageTokens.updateMultipleReservedTokens(
            _destinations,
            _num_tokens,
            _num_percents,
            _percent_decimals
