...
contract KRCICOContract is Ownable {
    ...
    function distributeFunds() internal {
        ...
        LongTermFoundationBudgetAccumulated = LongTermFoundationBudgetAccumulated
            .add(received.mul(LongTermFoundationBudgetPercentage).div(100));

        LegalContingencyFundsAccumulated = LegalContingencyFundsAccumulated.add(
            received.mul(LegalContingencyFundsPercentage).div(100)
        );

        MarketingAndCommunityOutreachAccumulated = MarketingAndCommunityOutreachAccumulated
            .add(
                received.mul(MarketingAndCommunityOutreachPercentage).div(100)
            );

        CashReserveFundAccumulated = CashReserveFundAccumulated.add(
            received.mul(CashReserveFundPercentage).div(100)
        );

        OperationalExpensesAccumulated = OperationalExpensesAccumulated.add(
            received.mul(OperationalExpensesPercentage).div(100)
        );

        SoftwareProductDevelopmentAccumulated = SoftwareProductDevelopmentAccumulated
            .add(received.mul(SoftwareProductDevelopmentPercentage).div(100));

        FoundersTeamAndAdvisorsAccumulated = FoundersTeamAndAdvisorsAccumulated
            .add(received.mul(FoundersTeamAndAdvisorsPercentage).div(100));
    }
    ...
    function validPurchase() internal constant returns (bool) {
        ...
        bool withinContributionLimit = msg.value >= minimumContribution &&
            msg.value <= maximumContribution;
     ...
     * can only be called from owner wallet
     **/
    function changeMinimumContribution(
     ...
     * can only be called from owner wallet
     **/
    function changeMaximumContribution(
